        ...
        stage('Checkout') {
            steps {
                git credentialsId: 'Github_Cred',
                    branch: 'deploying',
                    url: 'https://github.com/10kartikey/hospital-management-system.git'
            }
        }
            ...
            steps {
                script {
                    ...
                    sh """
                        ...
                        docker-compose logs --tail=10 hospital-app
                    """
                }
            }
        }
                ...
                withAWS(credentials: 'aws_cred', region: "${AWS_REGION}") {
                    script {
                        sh """
                            ...
                            kubectl set image deployment/hospital-app \\
                                hospital-app=${DOCKER_IMAGE}:${DOCKER_TAG} --record

 
                            echo "⏳ Waiting for deployments to complete..."
                            kubectl rollout status deployment/mongodb --timeout=300s
                            kubectl rollout status deployment/hospital-app --timeout=300s
                ...
                withAWS(credentials: 'aws_cred', region: "${AWS_REGION}") {
                    script {
                        sh """
                            ...
                            i=1
                            while [ \$i -le 10 ]; do
                                EXTERNAL_IP=\$(kubectl get service hospital-service -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || echo "")
                                EXTERNAL_HOSTNAME=\$(kubectl get service hospital-service -o jsonpath='{.status.loadBalancer.ingress[0].hostname}' 2>/dev/null || echo "")
                            ...
                            kubectl get service hospital-service
